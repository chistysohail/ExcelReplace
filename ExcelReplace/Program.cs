            ...
            try
            {
                ...
                using (var workbook = new XLWorkbook("path/to/your/excelfile.xlsx"))
                {
                    ...
                    foreach (var row in worksheet.RangeUsed().Rows())
                    {
                        ...
                        // Check if the word exists in the file (case-insensitive)
                        if (Regex.IsMatch(fileContent, Regex.Escape(findValue), RegexOptions.IgnoreCase))
                        {
                            Console.WriteLine($"Found '{findValue}' in the file. Replacing...");
                            // Replace in the TypeScript file content (case-insensitive)
                            fileContent = Regex.Replace(fileContent, Regex.Escape(findValue), replaceValue, RegexOptions.IgnoreCase);
                        }
                        else
                        {
